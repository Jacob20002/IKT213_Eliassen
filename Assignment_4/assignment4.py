...
def harris_corner_detection(reference_image_path):
    ...
    print("✅ Harris corners saved as 'harris.png'")
...
def align_images(image_to_align_path, reference_image_path, max_features=1000, good_match_percent=0.8):
    ...
    if ref_img is None:
        print(f"❌ Could not read reference image at: {reference_image_path}")
        return None
    if img is None:
        print(f"❌ Could not read image to align at: {image_to_align_path}")
        return None
    ...
    if descriptors1 is None or descriptors2 is None:
        print("❌ No descriptors found. Try adjusting image or parameters.")
        return None
    ...
    print("✅ Matches saved as 'matches.png'")

    # Need at least 4 good matches for homography
    if len(good_matches) < 4:
        print("❌ Not enough matches to align images. Try increasing max_features or good_match_percent.")
        return None
    ...
    print("✅ Aligned image saved as 'aligned.png'")
